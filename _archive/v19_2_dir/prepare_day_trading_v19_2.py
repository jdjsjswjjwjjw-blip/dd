...
ORIGINAL_FEATURES: tuple[str, ...] = (
    # ── تدفق الأوامر الحقيقي (من التيكات) ──
    'cvd', 'session_cvd', 'bar_cvd_delta',
    'absorption_intensity',                 # max + clip(0,20) — مُصلح
    'cancel_ratio', 'spoofing_ratio', 'spoofing_duration', 'liquidity_trap',
    # hawkes_intensity مُزالة (cumulative max معكوس) → hawkes_intrabar_sum أصح
    'hawkes_intrabar_sum',
    # kyle_lambda مُزالة (max مكسور) → kyle_lambda_intrabar_mean أصح
    'kyle_lambda_intrabar_mean',
    'vnet', 'vnet_intrabar_last',
    'micro_atr', 'volume_burst', 'inter_event_time',  # inter_event مع clip(3600)
    ...
    # ── MBP depth حقيقي (يُضاف من attach_mbp_depth_to_bars أو data_pipeline) ──
    'book_imbalance',    # (bid_sz - ask_sz)/(bid+ask) من MBP snapshot
    'depth_ratio',       # bid_depth / ask_depth
    'spread_mean',       # متوسط bid-ask spread داخل الشمعة
    # ── مستويات ──
    'pdh', 'pdl', 'dist_to_pdh', 'price_position',
    'london_sess_high', 'london_sess_low',
    'dist_to_london_high_atr', 'dist_to_london_low_atr',
    # ── بنية الشمعة ──
    'range_ratio',   # bar_range / avg_range_20 — كشف LR/HR
    'is_lr_bar',     # bar_range < 70% متوسط
    'is_hr_bar',     # bar_range > 130% متوسط
    'prev_is_lr',    # الشمعة السابقة كانت LR؟
...
# المُزالة: hawkes_intensity(معكوس), kyle_lambda(max مكسور),
#           absorption_std/cancel_std/kyle_std(مكررة بلا قيمة),
#           buy/sell_absorption_approx(=0 دائماً), liquidity_gaps(=0 دائماً),
#           mbp_roll_lob_coverage(=0 عملياً), vwap_dist(lookahead خطر),
#           return_*/volume_ratio_*/cvd_slope_*(مشتقات تسرب IC)
...
CATBOOST_ADVISOR_FEATURES_DT = [
    'cvd', 'session_cvd', 'bar_cvd_delta',
    'obi', 'absorption_intensity', 'cancel_ratio',
    ...
    'pdh', 'pdl', 'dist_to_pdh', 'price_position',
    'dist_to_london_high_atr', 'dist_to_london_low_atr',
    # hawkes/kyle: النسخ الصحيحة
    'hawkes_intrabar_sum', 'kyle_lambda_intrabar_mean',
    'vnet', 'vnet_intrabar_last',
    'vwap_z_score', 'vwap_dist_1h_roll',
    # MBP depth حقيقي
    'book_imbalance', 'depth_ratio', 'spread_mean',
    # بنية الشمعة
    'range_ratio', 'is_lr_bar', 'is_hr_bar', 'prev_is_lr', 'volume_ratio',
]
...
def _sanitize_bars_extreme_close_returns(df: pd.DataFrame, max_abs_pct_ret: float | None) -> pd.DataFrame:
    """
    تنظيف الشمعات ذات القفزات الشاذة:
    ① شمعات حيث |(close−open)/open| > max_abs_pct_ret  ← إسقاط
    ② شمعات حيث |close.pct_change()| > max_abs_pct_ret ← ffill بدل إسقاط
    """
    ...
    ret_oc = (cl - op) / base
    bad_oc = ret_oc.abs() > mx
    n_oc = int(bad_oc.sum())
    if n_oc:
        print(f"  🧹 sanitize bars (open-close): dropped {n_oc:,} rows with |(close-open)/open| > {mx:.4g}")
        df = df.loc[~bad_oc.fillna(False)].copy()

    # ② close-to-close outliers: ffill بدل إسقاط (تحافظ على continuity)
    cl2 = pd.to_numeric(df['close'], errors='coerce').astype(float)
    ret_cc = cl2.pct_change().abs()
    bad_cc = ret_cc > mx
    n_cc = int(bad_cc.sum())
    if n_cc:
        print(f"  🧹 sanitize bars (close-close): ffill {n_cc:,} rows with |close_pct_change| > {mx:.4g}")
        for col in ('open', 'high', 'low', 'close'):
            if col in df.columns:
                df.loc[bad_cc.fillna(False), col] = np.nan
...
def apply_bar_level_catboost_parities(df_bars: pd.DataFrame, freq: str) -> pd.DataFrame:
    ...
    for col in CATBOOST_ADVISOR_FEATURES_DT:
        ...
        if col == 'dist_to_pdh':
            df[col] = raw.clip(-20.0, 20.0).fillna(0.0).astype(np.float64)
        elif col == 'distance_to_wall':
            df[col] = raw.clip(0.0, 50.0).fillna(0.0).astype(np.float64)
        elif col == 'liquidity_density':
            p99 = raw.quantile(0.99)
            df[col] = raw.clip(0.0, float(p99) if np.isfinite(p99) else 1e6).fillna(0.0).astype(np.float64)
        elif col == 'absorption_intensity':
            df[col] = raw.clip(0.0, 20.0).fillna(0.0).astype(np.float64)
        elif col == 'inter_event_time':
            df[col] = raw.clip(0.0, 3600.0).fillna(0.0).astype(np.float64)
...
def aggregate_mbo_to_bars(df_mbo: pd.DataFrame, freq: str = DAY_TRADE_DEFAULT_BAR_FREQ) -> pd.DataFrame:
    ...
    # Order Flow — إصلاح aggregation المكسور
    # hawkes: delta (last-first) بدل max على cumulative ← إصلاح معكوس
    if 'hawkes_intensity' in df.columns:
        hk = pd.to_numeric(df['hawkes_intensity'], errors='coerce').fillna(0.0)
        bars['hawkes_intensity'] = hk.resample(freq).agg(
            lambda x: float(x.iloc[-1] - x.iloc[0]) if len(x) > 1 else 0.0
        ).reindex(bars.index).fillna(0.0).clip(lower=0.0).astype(np.float64)
    else:
        bars['hawkes_intensity'] = 0.0
    # hawkes_intrabar_sum: sum ← الأصح للنمذجة
    bars['hawkes_intrabar_sum'] = _resample_num('hawkes_intensity', 'sum', 0.0)

    # kyle_lambda: mean بدل max ← إصلاح
    bars['kyle_lambda'] = _resample_num('kyle_lambda', 'mean', 0.0)
    bars['kyle_lambda_intrabar_mean'] = _resample_num('kyle_lambda', 'mean', 0.0)

    # absorption_intensity: max + clip(0,20) ← إصلاح cap غير المنطقي
    bars['absorption_intensity'] = _resample_num('absorption_intensity', 'max', 0.0).clip(0.0, 20.0)

    bars['cancel_ratio'] = _resample_num('cancel_ratio', 'max', 0.0).clip(0.0, 1.0)
    ...
    for col, how in (
        ...
        rs = _tick_resample_optional(df, freq, col, how)
        if rs is not None:
            if col == 'inter_event_time':
                bars[col] = pd.to_numeric(rs, errors='coerce').clip(0.0, 3600.0)
            elif col == 'distance_to_wall':
                bars[col] = pd.to_numeric(rs, errors='coerce').clip(0.0, 50.0)
            else:
                bars[col] = rs
...
def add_day_trading_features(df: pd.DataFrame, freq: str = DAY_TRADE_DEFAULT_BAR_FREQ) -> pd.DataFrame:
    ...
    # ── بنية الشمعة: LR/HR + volume ratio ─────────────────────────
    hl_s  = pd.to_numeric(df['high'], errors='coerce') - pd.to_numeric(df['low'], errors='coerce')
    avg_rng = hl_s.rolling(20, min_periods=5).mean().clip(lower=1e-9)
    df['range_ratio'] = (hl_s / avg_rng).clip(0.0, 5.0).fillna(1.0).astype(np.float32)
    df['is_lr_bar']   = (df['range_ratio'] < 0.70).astype(np.int8)
    df['is_hr_bar']   = (df['range_ratio'] > 1.30).astype(np.int8)
    df['prev_is_lr']  = df['is_lr_bar'].shift(1).fillna(0).astype(np.int8)
    vol_s   = pd.to_numeric(df['volume'], errors='coerce').fillna(0)
    vol_avg = vol_s.rolling(20, min_periods=5).mean().clip(lower=1.0)
    df['volume_ratio'] = (vol_s / vol_avg).clip(0.0, 10.0).fillna(1.0).astype(np.float32)

    # ── MBP depth: يُضاف من data_pipeline أو attach_mbp_depth_to_bars ──
    # لو لم يُضف بعد، نضع قيم افتراضية حتى لا يُكسر finalize_daytrade_parquet_export
    for col_mbp in ('book_imbalance', 'depth_ratio', 'spread_mean'):
        if col_mbp not in df.columns:
            df[col_mbp] = np.float32(0.0)
...
def attach_mbp_depth_features(
    ...
    *,
    ...
    """
    يستخرج المعلومة الحقيقية من MBP — بدل coverage فقط:

    book_imbalance: (bid_sz_total - ask_sz_total) / (bid + ask)
      ∈ [-1, 1] — الأهم: يكشف أين الضغط الحقيقي في الدفتر

    depth_ratio: bid_depth / ask_depth
      > 1 = دعم أكثر من مقاومة

    spread_mean: متوسط (ask_px_00 - bid_px_00) داخل الشمعة
      يقيس تكلفة التداول وسيولة السوق

    يُستخدم peak-imbalance snapshot لكل شمعة
    (الـ snapshot ذو أقصى |imbalance| = أقوى لحظة معلوماتية)
    """
    out = df_bars.copy()

    # قيم افتراضية لو لا يوجد MBP
    out['book_imbalance'] = np.float32(0.0)
    out['depth_ratio']    = np.float32(1.0)
    out['spread_mean']    = np.float32(0.0)
    ...
    def _arr(col: str) -> np.ndarray:
        if col in mbp.columns:
            return pd.to_numeric(mbp[col], errors='coerce').fillna(0).to_numpy(dtype=np.float32)
        return np.zeros(len(mbp), dtype=np.float32)

    bid_sz = np.column_stack([_arr(c) for c in bid_sz_cols]) if all(c in mbp.columns for c in bid_sz_cols) else None
    ask_sz = np.column_stack([_arr(c) for c in ask_sz_cols]) if all(c in mbp.columns for c in ask_sz_cols) else None
    bid_px = np.column_stack([_arr(c) for c in bid_px_cols]) if all(c in mbp.columns for c in bid_px_cols) else None
    ask_px = np.column_stack([_arr(c) for c in ask_px_cols]) if all(c in mbp.columns for c in ask_px_cols) else None

    if bid_sz is None or ask_sz is None:
        return out

    mbp_ts = mbp['ts_event'].to_numpy(dtype='datetime64[ns]')
    bar_ts  = pd.to_datetime(out['ts_event']).to_numpy(dtype='datetime64[ns]')
    freq_ns = int(_bar_period_seconds(freq) * 1e9)

    book_imb_arr = np.zeros(len(out), dtype=np.float32)
    depth_r_arr  = np.ones(len(out),  dtype=np.float32)
    spread_arr   = np.zeros(len(out), dtype=np.float32)
    ...
    for i in range(len(out)):
        ...
        sl_bid = bid_sz[lo:hi].sum(axis=1)
        sl_ask = ask_sz[lo:hi].sum(axis=1)
        tot    = sl_bid + sl_ask + 1e-9
        imb    = (sl_bid - sl_ask) / tot

        # أقوى snapshot بأعلى |imbalance|
        best = int(np.argmax(np.abs(imb)))
        book_imb_arr[i] = float(np.clip(imb[best], -1.0, 1.0))
        b = float(sl_bid[best])
        a = float(sl_ask[best])
        depth_r_arr[i] = float(np.clip(b / max(a, 1e-9), 0.1, 10.0))
        ...
        if bid_px is not None and ask_px is not None:
            ...
            valid = (b0 > 0) & (a0 > b0)
            if valid.any():
                spread_arr[i] = float(np.mean(a0[valid] - b0[valid]))

    out['book_imbalance'] = book_imb_arr
    out['depth_ratio']    = depth_r_arr
    out['spread_mean']    = spread_arr

    n_filled = int((book_imb_arr != 0).sum())
    print(f"  ✅ MBP depth features: {n_filled}/{len(out)} bars (book_imbalance, depth_ratio, spread_mean)")
    return out
    ...
    *,
    ...
    if preflight_parquet:
        ...
        if df_mbp is not None and len(df_mbp):
            ...
            print("\n📊 استخراج عمق السوق الحقيقي من MBP...")
            df_bars = attach_mbp_depth_features(df_bars, df_mbp, freq)
        else:
            df_bars['mbp_bar_coverage'] = np.float32(0.0)
            df_bars['book_imbalance']   = np.float32(0.0)
            df_bars['depth_ratio']      = np.float32(1.0)
            df_bars['spread_mean']      = np.float32(0.0)
